...
function BindData(nPage) {
    CS('CZCLZ.SFKMag.GetFKList', function (retVal) {
        FkxmStore.setData({
            data: retVal.dt,
            pageSize: pageSize,
            total: retVal.ac,
...
//************************************页面方法***************************************
function xg(id) {
    CS('CZCLZ.SFKMag.GetFkxmById', function (retVal) {
        if (retVal) {
            var win = new addWin();
...
function del(id) {
    Ext.MessageBox.confirm("提示", "是否删除你所选?", function (obj) {
        if (obj == "yes") {
            CS('CZCLZ.SFKMag.DeleteFkxm', function (retVal) {
                if (retVal) {
                    BindData(1);
                }
            }, CS.onError, id);
                ...
                buttons: [
                    {
                        ...
                        handler: function () {
                            ...
                            if (form.form.isValid()) {
                                ...
                                var me = this;
                                CS('CZCLZ.SFKMag.SaveFkxm', function (retVal) {
                                    if (retVal) {
                                        Ext.Msg.show({
                                            title: '提示',
                                            msg: '保存成功!',
