...
//-----------------------------------------------------------数据源-------------------------------------------------------------------
//核销store
var companyStore = createSFW4Store({
    data: [],
    pageSize: pageSize,
    total: 1,
    currentPage: 1,
...
function getList(nPage) {
    var companyName = Ext.getCmp("cx_company").getValue();
    var bscid = Ext.getCmp("cx_bsc").getValue();
    var kssj = Ext.getCmp("start_time").getValue();
    var jssj = Ext.getCmp("end_time").getValue();

    CS('CZCLZ.Finance.GetCompanyHxByPage', function (retVal) {
        companyStore.setData({
            data: retVal.dt,
            pageSize: pageSize,
            total: retVal.ac,
            currentPage: retVal.cp
        });
    }, CS.onError, nPage, pageSize, bscid, kssj, jssj, companyName);
}

//操作-核销
function HX(zhongzhuanId) {
    var win = new HXWin();
    win.show(null, function () {
        Ext.getCmp('zhongzhuanId').setValue(zhongzhuanId);
        if (tabN == 1) {
            getWhxList(1);
        } else if (tabN == 2) {
            getYhxList(1);
...
function getWhxList(nPage) {
    var id = Ext.getCmp('zhongzhuanId').getValue();
    CS('CZCLZ.Finance.GetZZListByPage', function (retVal) {
                        ...
                        buttons: [
                            {
                                ...
                                iconCls: "save",
                                handler: function () {
                                    if (privilege("财务应付核销_办事处中转核销_核销")) {
                                        var xzlist = [];
                                        ...
                                        CS('CZCLZ.Finance.SaveZZHx', function (retVal) {
                                            if (retVal) {
                                                Ext.Msg.show({
                                                    title: '提示',
                                                    msg: '保存成功!',
                                        ...
                                        menu: [
                                            {
                                                text: "导出excel",
                                                handler: function () {
                                                    if (privilege("财务应付核销_办事处中转核销_导出")) {
                                                        var xzlist = [];
                                                        if (tabN == 1) {
                                                        ...
                                                        DownloadFile("CZCLZ.Finance.DownLoadBsczz", "办事处中转费核销.xls", xzlist);
                                                    }
                                                }
                                            },
                                            {
                    ...
                    dockedItems: [
                        {
                            ...
                            items: [
                                ...
                                },
                                {
                                    ...
                                    text: '查询',
                                    handler: function () {
                                        if (privilege("财务应付核销_办事处中转核销_查询")) {
                                            getList(1);
                                        }
                                    }
                                }
